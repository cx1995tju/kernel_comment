...
struct vsock_sock { //动态模块注册的时候会设置好相应函数指针？？？？
	/* sk must be the first member. */
	struct sock sk; //所有网域的sock的通用部分
	 ...
	 */
	struct list_head pending_links; //connect连接到来的时候，新建的socket放置在这个链上
	struct list_head accept_queue; //三次握手完成后，挂在这里，等待accept
	bool rejected;
