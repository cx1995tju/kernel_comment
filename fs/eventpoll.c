 ...
 */
/* 该结构的初始化发生在epoll_ctl(ADD) -> ep_insert()  */
struct epitem {
	/* eventpoll会使用一颗红黑树来管理所有的其监听的epitem对象 */
	union {
		/* RB tree node links this structure to the eventpoll RB tree */
		struct rb_node rbn; /* 链入到eventpoll的红黑树中，被其管理起来 */
	...
	/* 链入ready list */
	struct list_head rdllink; /* 链入到eventpoll的就绪队列中 */
	...
	/* 会将eppoll_entry结构链入到这个表中, 详见ep_ptable_queue_proc() */
	struct list_head pwqlist;

	/* The "container" of this item */
	/* 显然，指向eventpoll结构的 */
	struct eventpoll *ep;  /* 表示监听的事件 */
...
struct eventpoll {
	...
	struct list_head rdllist; //就绪的fd, 所有就绪的fd的list

	/* RB tree root used to store monitored fd structs */
	struct rb_root_cached rbr; //红黑树组织所有被监听的fd, 所有被监听的fd组织成红黑树
	 ...
	 * holding ->wq.lock.
	 */
	/* 维护一些在epoll_wait()处理过程中产生的事件epitem
	 *
	 * */
	...
	struct file *file; /* 该eventpoll本身也被vfs管理，这是对应的file结构，file->private 就是指向的该eventpoll结构 */
 ...
 * */
struct eppoll_entry {  /* 挂入socekt wait list的结构 */
	 ...
	 * queue head.
	 */
	/* ep_poll_callback()函数挂载在这里 */
	wait_queue_entry_t wait;

	/* The wait queue head that linked the "wait" wait queue item */
	/* 指向socket的sk_wq成员，其他类型的文件会不同的 */
	wait_queue_head_t *whead;
};

/* Wrapper struct used by poll queueing */
/* 一般来说，是一个临时的辅助结构, 将一个函数与一个epi项暂时关联起来 */
struct ep_pqueue {
	poll_table pt; //指向一个函数
	struct epitem *epi; //一个事件项
};
...
static __poll_t ep_item_poll(const struct epitem *epi, poll_table *pt,
				 int depth) //获取目标文件的当前产生的事件位，并且将监听项绑定到目标文件的钩子上
{
	struct eventpoll *ep;
	bool locked;

	pt->_key = epi->event.events; //设置了_key
	if (!is_file_epoll(epi->ffd.file)) //对于这个事件关联的文件不是epoll文件都来到了这里，也就是说大部分场景是来到了这里。
		return vfs_poll(epi->ffd.file, pt) & epi->event.events; //返回产生的事件项, 一定要与业务端要求监听的事件有交集
			//vfs_poll只是单纯的返回socket上产生的所有事件, 同时执行更具体的poll函数，在这个poll函数里
			//一般会执行poll_wait()函数
 ...
 * 底层的设备文件有事件发生后，通过我们设置的callback函数，调用到这里
 *
 * 处理就绪事件，唤醒对应进程
 */
static int ep_poll_callback(wait_queue_entry_t *wait, unsigned mode, int sync, void *key)
{
	...
	__poll_t pollflags = key_to_poll(key); //事件触发后，传入的事件参数, 但是我们在epoll_wait中会再次去请求mask的。
	int ewake = 0;
	...
	/* If this file is already in the ready list we exit soon */ //处理就绪队列问题，将epi链入对应的eventpoll的rdlist中
	if (!ep_is_linked(epi)) {
		list_add_tail(&epi->rdllink, &ep->rdllist);
...
{ /* 在socket文件的场景下，file是被监听socket文件， whead是socket的sk_wq, pt是指向ep_ptable_proc函数的*/
	...
	if (epi->nwait >= 0 && (pwq = kmem_cache_alloc(pwq_cache, GFP_KERNEL))) {
		init_waitqueue_func_entry(&pwq->wait, ep_poll_callback);  /* 这个ep_poll_callback 是核心的核心， socket被唤醒后就执行这个函数 */
		pwq->whead = whead; // whead结构是sk_sleep(sk), 即sk_wq
		pwq->base = epi; //关联到epi结构，就能关联到对应的file结构。最后通过pwq来回调ep_poll_callback()
		if (epi->event.events & EPOLLEXCLUSIVE)
			add_wait_queue_exclusive(whead, &pwq->wait); /* 将epoll_entry结构挂到了sk_wq上 */
		else
			add_wait_queue(whead, &pwq->wait); //pwq->wait 链入到 whead中
...
/* 核心工作，创建epitem对象，加入到eventpoll机制中监听 */
		     ...
		     struct file *tfile, int fd, int full_check)
{
	...
	__poll_t revents;
	long user_watches;
	struct epitem *epi; //根据epoll_event event来构造epitem结构，表示监听项，与epoll_event有较大差别（包含对应的fd等结构）
	struct ep_pqueue epq; //局部变量，临时结构, 一个函数 + epitem指针
	...
	if (epi->event.events & EPOLLWAKEUP) {
		...
		RCU_INIT_POINTER(epi->ws, NULL); /* 唤醒源置空 */
	}

	/* Initialize the poll table using the queue callback */
	/* 初始化epq, 这是一个临时结构咯，将epi与一个函数关联起来（ep_ptable_queue_proc） */
	epq.epi = epi;
	init_poll_funcptr(&epq.pt, ep_ptable_queue_proc); //epq结构的赋值，poll_wait()的时候会调用这个函数
	 ...
	 */
	ep_rbtree_insert(ep, epi); //将对应的eitem结构插入eventpoll的红黑树中
	...
	if (revents && !ep_is_linked(epi)) { //已经有事件了, 而且epi还没有链入到红黑树
		list_add_tail(&epi->rdllink, &ep->rdllist);
		ep_pm_stay_awake(epi); //又涉及到电源管理了，以及一些唤醒源的问题
	...
	/* We have to call this outside the lock */
	/* pwake 记录是否ep->poll_wait是否有wait entry */
	if (pwake)
		ep_poll_safewake(&ep->poll_wait);
			       ...
			       void *priv)
{
	...
	for (esed->res = 0, uevent = esed->events;
		...
		list_del_init(&epi->rdllink); //从ready list中卸除下来

		revents = ep_item_poll(epi, &pt, 1); //注意这儿的pt.func == NULL，所以最后不会进入到poll_wait函数的
	...
	/* 如果不是监听的事件产生了，就到这里返回了，ep->rdlist就还是空*/
	return 0;
}
...
{
	int res = 0, eavail, timed_out = 0; //注意不是timeout
	u64 slack = 0;
	wait_queue_entry_t wait;
...
fetch_events:
	...
	if (!ep_events_available(ep)) { //如果ready list中有事件，就不进入这个if
		/*
		 ...
		 */
		init_waitqueue_entry(&wait, current); //在wait中加入了当前进程，构建了一个后续需要使用的wait_entry
		__add_wait_queue_exclusive(&ep->wq, &wait); //添加到ep结构中, 至此进程就与event poll结构联系起来了
		...
		for (;;) {
			 ...
			 */
			set_current_state(TASK_INTERRUPTIBLE); //准备睡眠
			/*
			...
			if (ep_events_available(ep) || timed_out)
				break; //这里才是正常跳出循环，但是睡眠醒来后，会先被设置成TASK_INTERRUPTIBLE状态的，所以1826行要重新设置
			if (signal_pending(current)) { //有信号处理就提前终止系统调用
				res = -EINTR;
				break;
			}
		...
		__set_current_state(TASK_RUNNING); //返回后的就要在这里设置状态
	}
check_events:
	/* Is it worth to try to dig for events ? */ //有必要再检查？？？？
	 ...
	 * more luck.
	 */
	if (!res && eavail && /* ep_send_events() 是核心， 很多epoll的逻辑在这里实现的，譬如：oneshot， ET LT逻辑 */
...
{
	int error, fd;
	struct eventpoll *ep = NULL; //核心结构，做为epoll这个特殊文件系统的文件存在，当然需要消耗一个fd了
	struct file *file;
	...
	file = anon_inode_getfile("[eventpoll]", &eventpoll_fops, ep,
				 O_RDWR | (flags & O_CLOEXEC)); //将ep结构纳入到vfs中
	if (IS_ERR(file)) {
		error = PTR_ERR(file);
		goto out_free_fd;
	}
...
SYSCALL_DEFINE4(epoll_ctl, int, epfd, int, op, int, fd,
		struct epoll_event __user *, event) //eventpoll对象， op操作， fd被监控文件，event待处理的事件
{
	int error;
	int full_check = 0;
	...
	error = -EBADF;
	f = fdget(epfd); //获取对应的eventpoll文件的fd结构
	if (!f.file)
		goto error_return;

	/* Get the "struct file *" for the target file */
	tf = fdget(fd); //获取待监听的fd结构
	if (!tf.file)
		goto error_fput;

	/* The target file descriptor must support poll */
	error = -EPERM;
	if (!file_can_poll(tf.file)) //file中的poll函数指针不为空, 某个文件能够被poll的前提是其提供了poll函数，会被epoll机制使用，譬如：inet中的tcp_poll, vsock中的vsock_poll
		goto error_tgt_fput;

	/* EPOLLAKEUP 与系统的电源管理，自动休眠有关
	 *
	 ...
	 */
	error = -EINVAL;
	if (f.file == tf.file || !is_file_epoll(f.file)) //监听的目标文件就是epoll文件本身 || 传入的不是一个epoll文件
		goto error_tgt_fput;
	 ...
	 */
	if (ep_op_has_event(op) && (epds.events & EPOLLEXCLUSIVE)) { //不支持的一些操作
		if (op == EPOLL_CTL_MOD)
			goto error_tgt_fput;
	 ...
	 * our own data structure.   //NO CHECK
	 */
	ep = f.file->private_data;  //指向eventpoll结构
	 ...
	 */
	/* L2125 mutex_lock_nested(&ep->mtx, 0); */
	/* epitem 类型 */
	epi = ep_find(ep, tf.file, fd); //在红黑树中找目标文件的监听项，没有的话，就返回空
			 ...
			 int maxevents, int timeout)
{
	...
	f = fdget(epfd); //是eventpoll文件的file*
	if (!f.file)
		return -EBADF;
	 ...
	 */
	ep = f.file->private_data; //获取eventpoll*结构

	/* Time to fish for events ... */ //钓鱼ing
...
SYSCALL_DEFINE4(epoll_wait, int, epfd, struct epoll_event __user *, events,
		int, maxevents, int, timeout) //eventpoll对象，用户空间用于接收事件，
{
