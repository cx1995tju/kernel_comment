...
lookup_protocol:
	...
	err = -ENOBUFS;
	sk = sk_alloc(net, PF_INET, GFP_KERNEL, answer_prot, kern); // _answer_prot_
	if (!sk)
		goto out;
			  ...
			  int addr_len, int flags, int is_sendmsg) //fast open的syn包带数据 tcp_sendmsg_fastopen -> __inet_stream_connect fastopen场景下，不需要调用connect
{
	...
	switch (sock->state) { //socket state与tcp state还是不同的
	default:
		err = -EINVAL;
		goto out;
	case SS_CONNECTED:
	...
	case SS_UNCONNECTED:
		err = -EISCONN;
		if (sk->sk_state != TCP_CLOSE) //三次握手还没有开始的
			goto out;
	...
	/* 如果出于这两个状态之一的话，因为L675行调用了sk->sk_prot->connect()触发了三次握手的 */
	if ((1 << sk->sk_state) & (TCPF_SYN_SENT | TCPF_SYN_RECV)) {
		...
		//nonblock 形式的话，会进入这里然后出去，换句话说，就不会执行inet_wait_for_connect的，短路或
		...
		bool kern)
{
	...
	int err = -EINVAL;
	//inet_csk_accept
	struct sock *sk2 = sk1->sk_prot->accept(sk1, flags, &err, kern); //create a new sock structure, the corresponding socket structure was allocate at sys_accept4()
...
int inet_getname(struct socket *sock, struct sockaddr *uaddr,
			int peer) //peer == 0, sockname; peer==1, peername
{
	struct sock *sk		= sock->sk;
	struct inet_sock *inet	= inet_sk(sk);
	DECLARE_SOCKADDR(struct sockaddr_in *, sin, uaddr); // struct sockaddr_in *sin = (struct sockaddr_in*)uaddr
...
int inet_sendmsg(struct socket *sock, struct msghdr *msg, size_t size) //the buffer in msg is in user space
{
	struct sock *sk = sock->sk;
		 ...
		 int flags)
{
	...
	if (likely(!(flags & MSG_ERRQUEUE))) //从err queue里拿数据
		sock_rps_record_flow(sk);
...
int inet_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	...
	case SIOCSIFBRDADDR: //获取广播地址
	case SIOCSIFNETMASK: //获取子网掩码
	case SIOCSIFDSTADDR: //设置点到点地址
	case SIOCSIFPFLAGS: //设置网络标志扩展
	...
	default:
		if (sk->sk_prot->ioctl)
			err = sk->sk_prot->ioctl(sk, cmd, arg); //调用协议相关的ioctl试一试
		else
			err = -ENOIOCTLCMD;
		break;
	}
